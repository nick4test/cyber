...
@router.post("/AWSResource", response_class=JSONResponse)
async def dashboard(
    request: Request,
    ...
    if access_key:
        ...
        return await crud.check_aws_credentials(AWSCredentials(aws_access_key=aws_access_key, aws_secret_key=aws_secret_key, aws_region=aws_region))
    elif authorization:
        ...
        if not aws_access_key or not aws_secret_key or not aws_region:
            raise HTTPException(status_code=400, detail="AWS access key, secret key and region required")
        return await crud.check_aws_credentials(aws_access_key, aws_secret_key, aws_region)               
    else:
        raise HTTPException(
            status_code=401,
